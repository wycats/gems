...
class Gems::Package::TarWriter
  ...
  def add_file(name, mode) # :yields: io
    ...
    raise Gems::Package::NonSeekableIO unless @io.respond_to? :pos=
    ...
    header = Gems::Package::TarHeader.new :name => name, :mode => mode,
  ...
  def add_file_digest name, mode, digest_algorithm # :yields: io
    ...
    add_file name, mode do |io|
      Gems::Package::DigestIO.wrap io, digest do |digest_io|
        yield digest_io
      end
    end
  ...
  def split_name(name) # :nodoc:
    raise Gems::Package::TooLongFileName if name.size > 256
    ...
    if name.size <= 100 then
      ...
      if name.size > 100 or prefix.size > 155 then
        raise Gems::Package::TooLongFileName
      end
    end
